...
public class PrecisionMetronome_v4_MultipleDisplays : MonoBehaviour {
    ...
    void Update() {
        ...
        if (currentDspTime >= _nextBeatTime) {
            ...
            // ONLY schedule next beat if this isn't the first beat
            if (!_isFirstBeat) {  // ← ADD THIS CHECK
                ScheduleNextBeat();
            }

            _isFirstBeat = false;  // Reset the flag after using it
    ...
    private void ScheduleNextBeat() {
        // ADD THIS DEBUG LINE
        Debug.Log($"[{gameObject.name}] ScheduleNextBeat called - _nextBeatTime: {_nextBeatTime:F4}, AudioSettings.dspTime: {AudioSettings.dspTime:F4}");
